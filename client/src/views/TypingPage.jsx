...
export default function TypingPage({ base_url }) {
      ...
      try {
        ...
        if (data.length > 0) {
          const randomParagraph =
            data[Math.floor(Math.random() * data.length)].paragraph;
  ...
  useEffect(() => {
    ...
    socket.on("players-update", (playersList) => {
      const filteredPlayers = playersList.filter(
        (p) => p.name !== name || p.socketId === socket.id
      );
  ...
  const allPlayersReady =
    players.length > 0 && readyPlayers.size === players.length;
              ...
              {countdown ? (
                ...
                <>
                  <span className="text-lg font-bold">
                    {isGameActive
                      ? `Time: ${elapsedTime.toFixed(2)}s / 60s`
                  ...
                  {/* Modal notifications dengan style modern */}
